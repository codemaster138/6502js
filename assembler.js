...
function SimulatorWidget(node) {
  ...
  function Assembler() {
    ...
    function assembleDefs(lines) {
      ...
      var vals = [];
      var outlns = [];
      for (var c = 0; c < lns.length; c++){ //iterate the lines
        var ln = lns[c];
        wds = ln.split(" ");
        words = rmitems(wds, "");
        console.log("Look: "+ words);
        if (words[0] === "define") {
          defs.push(words[1]);
          vals.push(words[2]);
        }else{
          for (var w = 0; w < words.length; w++){
            if (defs.indexOf(words[w]) > -1) {
              words[w] = vals[defs.indexOf(words[w])];
            }else{
              var strng = words[w];
              if (defs.indexOf(strng.slice(1)) > -1){
                words[w] = strng.charAt(0) + vals[defs.indexOf(strng.slice(1))];
              }
            }
          }
          outlns.push(wds.join(" "));
        }
      }
      return outlns;
    }
    function rmitems(x, y) {
    ...
    function assembleCode() {
      ...
      console.log("I've gotcha some code: " + code);
      code += "\n\n";
      var lines = code.split("\n");
      codeAssembledOK = true;
      lines = assembleDefs(lines);
